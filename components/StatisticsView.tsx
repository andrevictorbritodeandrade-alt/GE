...
export const StatisticsView: React.FC<StatisticsViewProps> = ({ classData, onBack }) => {
    ...
    return (
      <div className="space-y-6 max-w-7xl mx-auto pb-16 font-sans">
        <ScreenHeader
          ...
          badge="ESTATÍSTICAS • CONSOLIDADO"
          statusBadge="RELATÓRIO GERAL"
          title="RELATÓRIO GERAL CONSOLIDADO"
          subtitle="Visão integrada de frequência escolar, médias por turma e dados mensais"
          actions={
            <button 
              onClick={() => window.print()}
              className="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl shadow-md text-xs font-black uppercase tracking-wider transition flex items-center gap-2"
            >
  ...
  if (viewMode === 'details' && selectedClassId && classData) {
    ...
    return (
      <div className="space-y-6 max-w-7xl mx-auto pb-16 font-sans">
        <ScreenHeader
          onBack={() => setViewMode(selectedGrade ? 'class_select' : 'full_report')}
          badge="HISTÓRICO INDIVIDUAL • FREQUÊNCIA"
          statusBadge={currentClass.name}
          title={`HISTÓRICO: ${currentClass.name}`}
          subtitle={`${currentClass.school || 'Unidade Escolar'} • Total de Alunos: ${currentClass.students?.length || 0}`}
          actions={
            <button 
              onClick={() => window.print()}
              className="px-5 py-2.5 bg-white border border-slate-300 text-slate-800 rounded-xl hover:bg-slate-50 text-xs font-black uppercase tracking-wider shadow-sm transition flex items-center gap-2"
            >
              <span>🖨️</span> Imprimir Turma
            </button>
          }
        />
  ...
  if (viewMode === 'class_select' && selectedGrade && classData) {
    ...
    return (
      <div className="space-y-6 max-w-7xl mx-auto pb-16 font-sans">
        <ScreenHeader
          onBack={() => setViewMode('school_select')}
          badge="HISTÓRICO POR ESCOLA"
          statusBadge={selectedGrade}
          title={selectedGrade}
  ...
  if (viewMode === 'school_select') {
    return (
      <div className="space-y-6 max-w-7xl mx-auto pb-16 font-sans">
        <ScreenHeader
          onBack={() => setViewMode('dashboard')}
          badge="HISTÓRICO POR ALUNO"
          statusBadge="SELEÇÃO DE ESCOLA"
          title="RELATÓRIO POR ESCOLA"
          subtitle="Selecione a unidade escolar para consultar o histórico individual por turma"
        />
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-16 font-sans">
      <ScreenHeader
        onBack={onBack}
        badge="GESTÃO PEDAGÓGICA • 2026"
        statusBadge="ESTATÍSTICAS"
        title="PAINEL DE ESTATÍSTICAS"
        subtitle="Métricas consolidadas de frequência, presença diária e gráficos de desempenho por turma"
      />
