            ...
            {slideAtual.content && !slideAtual.points && !slideAtual.topicos && (
              <div className="space-y-6 max-w-5xl">
                {slideAtual.content.split('\n').filter(Boolean).map((par: string, idx: number) => (
                  <p key={idx} className="text-2xl md:text-3xl font-bold text-slate-200 leading-relaxed text-justify indent-8">
                    {par}
                  </p>
                ))}
              </div>
            ...
            {hasDiagram ? (
                ...
                <div className="lg:col-span-6 flex flex-col justify-center">
                  ...
                  {slideAtual.content && !slideAtual.points && !slideAtual.topicos && (
                    <div className="space-y-4 max-w-xl">
                      {slideAtual.content.split('\n').filter(Boolean).map((par: string, idx: number) => (
                        <p key={idx} className="text-lg md:text-xl font-bold text-slate-700 leading-relaxed text-justify indent-8">
                          {par}
                        </p>
                      ))}
                    </div>
              ...
              <div className="flex flex-col justify-center">
                ...
                {slideAtual.content && !slideAtual.points && !slideAtual.topicos && (
                  <div className="space-y-6 max-w-5xl">
                    {slideAtual.content.split('\n').filter(Boolean).map((par: string, idx: number) => (
                      <p key={idx} className="text-xl md:text-2xl font-bold text-slate-700 leading-relaxed text-justify indent-8">
                        {par}
                      </p>
                    ))}
                  </div>
