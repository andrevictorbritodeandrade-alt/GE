...
export const PortalView: React.FC<PortalViewProps> = ({ onSelectAccess }) => {
  ...
  return (
    <div className="relative z-10 w-full min-h-screen flex flex-col justify-between p-4 md:p-8 overflow-y-auto custom-scrollbar bg-slate-50/70">
      ...
      {/* Top Header Section */}
      <div className="relative z-10 w-full max-w-4xl mx-auto flex flex-col items-center text-center mt-6 md:mt-10 py-4 animate-fade-in-down">
        
        {/* Artistic Hero Image */}
        <div className="relative w-28 h-28 md:w-36 md:h-36 mb-6 rounded-[2rem] overflow-hidden shadow-[0_20px_50px_rgba(59,130,246,0.15)] border-4 border-white transform rotate-3 hover:rotate-0 transition-transform duration-500 cursor-pointer bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center">
          {!imgError ? (
            <img 
              src={heroImage} 
              alt="Arte Conteúdos Teóricos" 
        ...
        {/* Badge */}
        <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-white/80 backdrop-blur-md border border-slate-200/60 shadow-[0_4px_20px_rgba(0,0,0,0.03)] mb-6">
        ...
        {/* Core Title */}
        <h1 className="text-4xl md:text-5xl lg:text-7xl font-black text-slate-900 tracking-tighter leading-[1.1] mb-6 drop-shadow-sm flex flex-col items-center">
          <span className="block text-4xl md:text-6xl lg:text-7xl">CONTEÚDOS TEÓRICOS</span>
          <span className="bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 text-transparent bg-clip-text uppercase mt-2 inline-block drop-shadow-sm text-2xl md:text-4xl lg:text-5xl font-black">
            Prof. André Brito
          </span>
        </h1>

        {/* Intro Subtitle */}
        <p className="text-sm md:text-lg text-slate-600 max-w-2xl font-medium leading-relaxed px-4">
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 my-8 px-2 md:px-4">
        {/* --- STUDENTS PORTAL PANEL --- */}
        <div
          onClick={() => onSelectAccess('alunos')}
          className="group relative overflow-hidden rounded-[2.5rem] bg-white border border-slate-200 hover:border-blue-400 shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:-translate-y-1.5 flex flex-col justify-between p-8 md:p-10 transition-all duration-300 cursor-pointer min-h-[350px]"
        >
        ...
        <div
          ...
          className="group relative overflow-hidden rounded-[2.5rem] bg-white border border-slate-200 hover:border-emerald-400 shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:-translate-y-1.5 flex flex-col justify-between p-8 md:p-10 transition-all duration-300 cursor-pointer min-h-[350px]"
        >
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between mt-auto mb-4 px-4 text-center md:text-left gap-4 text-xs font-mono text-slate-500 border-t border-slate-200 pt-6 font-medium">
        <div className="flex items-center gap-2">
