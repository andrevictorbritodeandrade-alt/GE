...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, currentView, setView, onSwitchToPortal }) => {
        ...
        <nav className="mt-8 px-4 space-y-2 flex-1 overflow-y-auto custom-scrollbar">
          {onSwitchToPortal && (
            <button
              ...
              className="flex items-center w-full px-5 py-3.5 mb-3 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-md hover:shadow-lg hover:scale-[1.02] transition-all"
            >
              <svg className="w-5 h-5 text-white mr-3 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
              </svg>
              <span className="font-black uppercase text-[11px] tracking-widest">Menu Iniciar</span>
            </button>
          )}
          {menuItems.map((item) => (
            <button
