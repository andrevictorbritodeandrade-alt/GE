...
export const GalleryView: React.FC<GalleryViewProps> = ({ onBack, data, setData }) => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-24 font-sans">
      <ScreenHeader
        onBack={onBack}
        badge="REGISTRO FOTOGRÁFICO • 2026"
        statusBadge="MÍDIA"
        title="GALERIA DE FOTOS"
        subtitle="Memórias visuais das práticas corporais, eventos escolares e projetos pedagógicos"
        actions={
          <button 
            onClick={() => setIsAddModalOpen(true)}
            className="px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-lg active:scale-95 transition-all flex items-center gap-2"
      ...
      <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 md:gap-3 bg-white/80 backdrop-blur-md p-4 rounded-3xl border border-slate-200 shadow-md">
        {images.map((img) => (
          <div 
            key={img.id}
