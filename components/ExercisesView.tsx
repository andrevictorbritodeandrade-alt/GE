  ...
  if (viewState === 'categories') {
    return (
      <div className="space-y-6 max-w-7xl mx-auto pb-16 font-sans">
        <ScreenHeader
          onBack={onBack}
          badge="JOGOS DE TABULEIRO • XADREZ"
          statusBadge="TREINO TÁTICO"
          title="EXERCÍCIOS DE XADREZ"
          subtitle="Exercícios táticos interativos, problemas de mate e estudo de abertura"
        />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {categories.map(cat => (
            <div 
              ...
              className="bg-white/90 backdrop-blur-md p-8 rounded-3xl flex items-center gap-6 cursor-pointer hover:bg-white transition-all transform hover:scale-[1.01] group border-l-8 border-blue-500 shadow-md hover:shadow-xl"
            >
              <div className="w-20 h-20 bg-blue-100 rounded-2xl flex items-center justify-center text-4xl shadow-inner group-hover:scale-110 transition-transform">
                {cat.icon}
              </div>
              <div>
                <h3 className="text-2xl font-black text-slate-800 uppercase">{cat.title}</h3>
                <p className="text-slate-500 font-medium">{cat.desc}</p>
                <span className="inline-block mt-3 text-xs font-bold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-md border border-blue-100">
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-16 font-sans">
      <ScreenHeader
        ...
        badge="XADREZ TÁTICO"
        statusBadge={categories.find(c => c.id === selectedCategory)?.title || "CATEGORIA"}
        title={categories.find(c => c.id === selectedCategory)?.title || "EXERCÍCIOS"}
        subtitle="Resolva o problema no tabuleiro arrastando as peças corretas"
      />
