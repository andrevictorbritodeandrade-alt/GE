...
const CLASS_PLAN_MAP: Record<string, string> = {
  ...
  '803': '8ano',
  'CIEP369': '8ano',
  'CE_IGNACIO_1001': 'ilgch',
  'CE_IGNACIO_2001': 'ilgch',
  'CE_IGNACIO_2002': 'ilgch',
  'CE_IGNACIO_1001_fri': 'ilgch',
  'CE_IGNACIO_2001_fri': 'ilgch',
  'CE_IGNACIO_2002_fri': 'ilgch',
  'CIEP476_1001': 'ilgch',
  'CIEP476_1002': 'ilgch',
  'CIEP476_2001': 'ilgch',
  'CIEP229_EJA': 'ejanem',
...
const CLASSES = {
  ...
  TUESDAY: [
    { id: 'CE_IGNACIO_2001', name: 'IFFC 2001 (CE Dr. Ignácio)', segment: SEGMENTS.MEDIO },
    { id: 'CE_IGNACIO_2002', name: 'IFLA 2002 (CE Dr. Ignácio)', segment: SEGMENTS.MEDIO },
    { id: 'CE_IGNACIO_1001', name: 'ILGCH 1001 (CE Dr. Ignácio)', segment: SEGMENTS.MEDIO },
  ],
  FRIDAY: [
...
const SCHEDULE_MONDAY = [
  { date: '11/05', type: 'class', activities: { 
    ...
    'CIEP369': { title: 'Apresentação e Provas', description: 'Conhecendo a turma.\n\nAplicação de provas de recuperação de outros professores.' }, 
  ...
  { date: '17/08', type: 'class', activities: { 
    ...
    'CIEP369': { ...AULA_LUTAS_BASE(), assignment: { type: 'passar', points: 3, deadline: '14/09', title: 'Construção de Jogos Africanos', text: 'Trazer um tabuleiro de Shisima ou Mancala construído em casa com material reciclável.'} }, 
  ...
  { date: '14/09', type: 'class', activities: { 
    ...
    'CIEP369': { ...AULA_SHISIMA_BASE(), assignment: { type: 'receber', points: 3, title: 'Receber: Tabuleiros Africanos'} }, 
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-16 font-sans">
      ...
      {modalData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm" onClick={() => setModalData(null)}>
          ...
          >
            {/* Cabeçalho do Modal */}
            <div className={`p-6 flex justify-between items-start border-b-2 borde-slate-700 ${modalData?.segment?.color ? modalData.segment.color.split(' ')[0].replace('bg-', 'bg-slate-900/50 text-') : 'bg-slate-900 text-white'}`}>
              <div>
            ...
            <div className="p-8 overflow-y-auto max-h-[60vh] bg-slate-900/50">
              ...
              <h4 className="text-3xl font-black text-white mb-8 flex items-center gap-4 border-b-2 border-slate-700 pb-4 tracking-tighter uppercase">
                 ...
                 {modalData?.activity?.title || 'Detalhes da Aula'}
              </h4>
                ...
                <div className={`mb-8 p-6 rounded-2xl border-2 shadow-xl ${modalData.activity.assignment.type === 'passar' ? 'bg-orange-950/30 border-orange-800' : 'bg-red-950/30 border-red-800'}`}>
                  <h5 className={`font-black text-xl flex items-center gap-3 tracking-tighter uppercase ${modalData.activity.assignment.type === 'passar' ? 'text-orange-200' : 'text-red-200'}`}>
                    ...
                    <span className="bg-slate-950 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm border border-slate-700 text-white ml-auto">
                      {modalData.activity.assignment.points || 3} pts
                    </span>
                  </h5>
                  
                  <div className="mt-4 text-slate-300">
                    <p className="font-black text-xl border-b-2 border-white/10 pb-3 mb-4 tracking-tight uppercase">{modalData.activity.assignment.title || modalData.activity.title}</p>
