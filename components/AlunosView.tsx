...
export const AlunosView: React.FC<AlunosViewProps> = ({ onBack, classData }) => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-16 font-sans print:bg-white print:text-black">
      {/* Header Padrão Unificado */}
      <div className="print:hidden">
        <ScreenHeader
          onBack={() => selectedMaterial ? setSelectedMaterial(null) : (selectedTurma ? setSelectedTurma(null) : onBack())}
          badge="PORTAL PEDAGÓGICO • ESTUDANTES"
          statusBadge={selectedTurma ? "TURMA SELECIONADA" : "DISPONÍVEL"}
          title={selectedTurma ? (TURMAS_ALUNOS.find(t => t.id === selectedTurma)?.school || "PORTAL DO ALUNO") : "PORTAL DO ALUNO"}
          subtitle={selectedTurma ? (TURMAS_ALUNOS.find(t => t.id === selectedTurma)?.title || "Material de estudo e resumos de aula") : "Acesse resumos, anotações de aula e materiais de estudo para recuperação e provas"}
        />
      </div>

      <div className="max-w-5xl w-full mx-auto p-2 md:p-4 flex-1">
