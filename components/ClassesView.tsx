  ...
  if (!selectedGrade) {
    return (
      <div className="space-y-6 max-w-7xl mx-auto pb-16 animate-fade-in">
        <ScreenHeader
          onBack={onBack}
          badge="GESTÃO ESCOLAR • 2026"
          statusBadge="SEEDUC-RJ"
          title="DIÁRIO DE CLASSE"
          subtitle="Controle de frequência, notas e lista de presença por unidade escolar"
        />

        <div className="bg-white/90 backdrop-blur-xl p-6 md:p-8 rounded-3xl shadow-xl border border-slate-200 text-slate-800 font-sans w-full relative overflow-hidden">
    ...
    return (
      <div className="space-y-6 max-w-7xl mx-auto pb-16 animate-fade-in">
        <ScreenHeader
          ...
          badge="UNIDADE ESCOLAR"
          statusBadge={`${classes.length} TURMAS`}
          title={selectedGrade || "DIÁRIO DE CLASSE"}
          subtitle="Selecione a turma desejada para abrir a lista de chamada e controle de frequência"
        />

        <div className="bg-white/90 backdrop-blur-xl p-6 md:p-8 rounded-3xl shadow-xl border border-slate-200 text-slate-800 font-sans w-full relative overflow-hidden">
          <div className="mb-6 border-b border-slate-200 pb-4">
            <h2 className="text-xl md:text-2xl font-black tracking-tight text-slate-900 uppercase">Turmas Disponíveis</h2>
            <p className="text-xs text-slate-500 font-medium">Selecione para gerenciar chamada, notas e atividades</p>
          </div>
  ...
  if (!currentClass) {
    return (
      <div className="p-8 text-center text-slate-500 space-y-4">
        <p className="text-lg font-bold">Turma não encontrada ou selecionada incorretamente.</p>
        <button onClick={() => setSelectedClassId(null)} className="px-4 py-2 bg-blue-600 text-white rounded-xl font-bold">Voltar às Turmas</button>
      </div>
    );
  }
  ...
  return (
    <>
      <ScreenHeader
        ...
        badge={currentClass.grade ? `${currentClass.grade}º ANO` : 'TURMA'}
        statusBadge={syncStatus === 'saving' ? 'SINCRONIZANDO...' : syncStatus === 'error' ? 'ERRO DE SYNC' : 'CONECTADO'}
        title={currentClass.name.toUpperCase()}
        subtitle={`${currentClass.school || ''} • Horário: ${currentClass.schedule || 'N/D'} • Dia: ${dayOfWeek}`}
        actions={
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex flex-col bg-slate-800/80 p-2 rounded-xl border border-white/10">
              <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Data da Chamada</span>
              <input 
                type="date" 
                value={selectedDate}
                onChange={(e) => setSelectedDate(e.target.value)}
                className="text-sm font-black text-white bg-transparent border-none p-0 focus:ring-0 cursor-pointer appearance-none outline-none"
              />
            </div>
            <button
              onClick={() => (window as any).setView('schedule')}
              className="px-3 h-10 flex items-center justify-center bg-white/10 border border-white/20 text-white text-xs font-black uppercase rounded-xl hover:bg-white/20 shadow-sm transition-all active:scale-95"
            >
              Quadro
            </button>
            <button
            ...
            <button
              ...
              className="w-10 h-10 flex items-center justify-center text-white bg-white/10 hover:bg-white/20 rounded-xl transition-all border border-white/20"
              title="Imprimir"
            >
            ...
            <button
              ...
              className="w-10 h-10 flex items-center justify-center text-white bg-white/10 hover:bg-amber-600/50 rounded-xl transition-all border border-white/20"
            ...
            <button 
              ...
              className="w-10 h-10 flex items-center justify-center text-white bg-white/10 hover:bg-emerald-600/50 rounded-xl transition-all border border-white/20"
            ...
            <button 
              ...
              disabled={isScanning}
              className={`w-10 h-10 flex items-center justify-center text-white bg-white/10 hover:bg-purple-600/50 rounded-xl transition-all border border-white/20 ${isScanning ? 'animate-pulse' : ''}`}
