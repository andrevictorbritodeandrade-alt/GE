...
export const slidesData: Record<string, Slide[]> = {
  ...
  'ap_01/06': [
    ...
    },
    {
      ...
      subtexto: 'Educação Física e Cidadania - SEEDUC-RJ',
...
export const DecolonialApp: React.FC<DecolonialAppProps> = ({ 
  ...
  const renderCard = (aula: any, index: number, corHeader: string, corBadge: string, turmaContext: string = 'ilgch', isCurrentClass: boolean = false) => {
              ...
              <>
                {pastAulas.length > 0 && (
                  <details className="mb-8 group/details">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6 p-2">
                      {pastAulas.map(renderAulaCard)}
                    </div>
                  </details>
                )}
