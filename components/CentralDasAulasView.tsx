...
export const CentralDasAulasView: React.FC<CentralDasAulasViewProps> = ({ onBack }) => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-16 font-sans">
      <ScreenHeader
        onBack={() => onBack('home')}
        badge="REPOSITÓRIO DIDÁTICO • 2026"
        statusBadge="AULAS INTERATIVAS"
        title="CENTRAL DE AULAS"
        subtitle="Slides, imagens e roteiros interativos para projeção em sala de aula"
        actions={
          <div className="flex items-center gap-2 flex-wrap">
            <select 
              value={segmento} 
              onChange={e => setSegmento(e.target.value)}
              className="border border-slate-200 rounded-xl px-3 py-2 bg-white text-slate-800 text-xs font-bold cursor-pointer focus:ring-2 focus:ring-sky-500 outline-none shadow-sm"
            >
